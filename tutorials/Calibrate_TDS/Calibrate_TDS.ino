...
#define EC_PIN 3// change to pin of station
...
#define ECSOLUTION 1.413 //CHANGE TO CALIB SOLUTION
...
//         if(analogBufferIndex == SCOUNT)
    ...
    for(copyIndex=0;copyIndex<SCOUNT;copyIndex++)  // for coppyIndex = 0 to SCOUNT-1
      ...
      rawECsolution = (ECSOLUTION*(1.0+0.0185*(temperature-25.0)))*1000;  //temperature compensation
...
//                  + 624.194*averageVoltage) /compensationCoefficient * K; //convert voltage value to SpC value, then correct for temp
      ...
      if((KValueTemp>0.25) && (KValueTemp<4))
      {
          Serial.println();

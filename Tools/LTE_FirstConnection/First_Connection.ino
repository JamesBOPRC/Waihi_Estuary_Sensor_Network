...
void loop() {
    ...
    if (gsmModem.commandMode()) {
        ...
        /** Enable pin sleep functionality on <code>DIO9</code>.
         * NOTE: Only the <code>DTR_N/SLEEP_RQ/DIO8</code> pin (9 on the bee socket) can be
         * used for this pin sleep/wake. */
        gsmModem.sendAT(GF("D8"), 1);
        gsmModem.waitResponse();
        /** Enable status indication on <code>DIO9</code> - it will be HIGH when the XBee
         * is awake.
         * NOTE: Only the <code>ON/SLEEP_N/DIO9</code> pin (13 on the bee socket) can be
         * used for direct status indication. */
        gsmModem.sendAT(GF("D9"), 1);
        gsmModem.waitResponse();
        /** Enable CTS on <code>DIO7</code> - it will be <code>LOW</code> when it is clear to send
         * data to the XBee.  This can be used as proxy for status indication if
         * that pin is not readable.
         * NOTE: Only the <code>CTS_N/DIO7</code> pin (12 on the bee socket) can be used
         * for CTS. */
        gsmModem.sendAT(GF("D7"), 1);
        gsmModem.waitResponse();
        /** Enable association indication on <code>DIO5</code> - this is should be directly
         ...
         * attempt
         *
         * NOTE: Only the <code>Associate/DIO5</code> pin (15 on the bee socket) can be
         * used for this function. */
        gsmModem.sendAT(GF("D5"), 1);
        gsmModem.waitResponse();
        /** Enable RSSI PWM output on <code>DIO10</code> - this should be directly attached
         * to an LED if possible.  A higher PWM duty cycle (and thus brighter
         * LED) indicates better signal quality.
         * NOTE: Only the <code>DIO10/PWM0</code> pin (6 on the bee socket) can be used for
         * this function. */
        ...
        gsmModem.sendAT(GF("CP"), 1); //change to 1 for Spark
        gsmModem.waitResponse();
        ...
        //set the band mask for spark
        DBG(F("Setting the Band Mask..."));
